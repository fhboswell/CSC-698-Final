...
void predict(float* w, float b, float* x, float* yProb, int N, int M, int NR, struct clDataOperation *kernelData);
...
void stageForLR(const float *Xvec, const float *Yvec, const int sizeX, const int sizeY , float *model){
   ...
   // *(model) = 5;
    ...
    //predict(w,*(b),X, Y, dw,  N,  M, NR,  &kernelData);
...
void stageForPredict(const float *Xvec, float *model, const int sizeX, const int sizeY , float *yPred){
    ...
    float *modell = (float *)calloc(N, sizeof(float));
    i = 0;
    j = 0;
    while(i < 12288) {
        float f = model[i];
  ...
  //  for (int i = 0; i <  MR; i ++){
   ...
   // }
    ...
    predict(modell,b,Xtest, yPred,  N,  M, NR,  &kernelData3);
    
    //predict(w,*(b),Xtest, ypredict, dw, db,  N,  M, NR, &kernelData3);
}
void predict(float* w, float b, float* x, float* yProb, int N, int M, int NR, struct clDataOperation *kernelData){
...
void startKernel( struct clDataOperation *kernelData, float * data, int M, int N, int NR ){
    ...
    kernelData->program = clCreateProgramWithSource(kernelData->context, 1, (const char **) & KernelSource2, NULL, &err);
    clBuildProgram(kernelData->program, 0, NULL, NULL, NULL, NULL);
    kernelData->kernel = clCreateKernel(kernelData->program, "gemv", &err);
    ...
    clEnqueueWriteBuffer(kernelData->commands, kernelData->input, CL_TRUE, 0, sizeof(float) *  M * N, data, 0, NULL, NULL);
    clSetKernelArg(kernelData->kernel, 0, sizeof(cl_mem), &kernelData->input);
    clSetKernelArg(kernelData->kernel, 1, sizeof(cl_mem), &kernelData->input2);
    clSetKernelArg(kernelData->kernel, 2, sizeof(cl_mem), &kernelData->output);
    clSetKernelArg(kernelData->kernel, 3, sizeof(unsigned int), &NR);
    clSetKernelArg(kernelData->kernel, 4, sizeof(int), &M);
    clSetKernelArg(kernelData->kernel, 5, sizeof(int), &N);
...
void startKernel2( struct clDataOperation *kernelData, float * data, int M, int N, float NR){
    ...
    kernelData->commands = clCreateCommandQueue(kernelData->context, kernelData->device_id, 0, NULL);
    cl_event event = NULL;
    kernelData->program = clCreateProgramWithSource(kernelData->context, 1, (const char **) & KernelSource3, NULL, &err);
    clBuildProgram(kernelData->program, 0, NULL, NULL, NULL, NULL);
    kernelData->kernel = clCreateKernel(kernelData->program, "gemvt", &err);
    ...
    clSetKernelArg(kernelData->kernel, 1, sizeof(cl_mem), &kernelData->input2);
    clSetKernelArg(kernelData->kernel, 2, sizeof(cl_mem), &kernelData->output);
    clSetKernelArg(kernelData->kernel, 3, sizeof(unsigned int), &count);
    clSetKernelArg(kernelData->kernel, 4, sizeof(int), &M);
    clSetKernelArg(kernelData->kernel, 5, sizeof(int), &N);
    clSetKernelArg(kernelData->kernel, 6, sizeof(float), &NR);
}
...
int dotmv(float * data, float * data2, int M, int N, float * results, int DATA_SIZE, float b, int NR, struct clDataOperation *kernelData){
    ...
    clSetKernelArg(kernelData->kernel, 6, sizeof(float), &b);
    
    size_t global[2] = {count, 256 };
    size_t  local[2] = {1, 256};
    
    clEnqueueNDRangeKernel(kernelData->commands, kernelData->kernel, 2, NULL, global, local, 0, NULL, NULL);
    
    clFinish(kernelData->commands);
    clEnqueueReadBuffer( kernelData->commands, kernelData->output, CL_TRUE, 0, sizeof(float) * M, results, 0, NULL, NULL );
    return 0;
}
...
int costVec(float * data, float * data2, float * results, int DATA_SIZE){
    ...
    kernel = clCreateKernel(program, "cost", &err);
    ...
    clEnqueueWriteBuffer(commands, input2, CL_TRUE, 0, sizeof(float) * count, data2, 0, NULL, NULL);
